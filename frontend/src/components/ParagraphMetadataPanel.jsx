...
/**
 * ParagraphMetadataPanel - floating metadata editor for a paragraph.
 */
const ParagraphMetadataPanel = ({
  isOpen,
  onClose,
  paragraph,
  onSave,
  ...
  const initialRows = useMemo(() => {
    if (Array.isArray(paragraph?.metadata) && paragraph.metadata.length > 0) {
      return paragraph.metadata.map((row) => createRow(row));
    }
    if (paragraph?.metadata && typeof paragraph.metadata === 'object') {
      return Object.entries(paragraph.metadata).map(([key, value]) =>
        createRow({ key, value })
      );
    }
    return [createRow()];
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-6 top-24 w-[320px] max-w-[90vw] rounded-xl border border-gray-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          <div>
            <div className="text-sm font-semibold text-gray-800">Paragraph metadata</div>
            <div className="text-xs text-gray-500">
              {paragraph?.topic ? `Topic: ${paragraph.topic}` : 'No topic set'}
            </div>
          </div>
          <button
            type="button"
...
export default ParagraphMetadataPanel;
